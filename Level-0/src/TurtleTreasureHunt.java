...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if(tortoiseLocationX == 500&&tortoiseLocationY == 300){
			JOptionPane.showMessageDialog(null, "If you are to find the treasure you see,"
					+ " you must go to the head who's eye is in a tree.");
			
			
		}
		if((tortoiseLocationX>=100&&tortoiseLocationX<=220)&&(tortoiseLocationY>=20&&tortoiseLocationY<=100)){
			JOptionPane.showMessageDialog(null, "If you are in search of a burried chest, you must find the boy wearing a blue vest.");
	
	}
		if(tortoiseLocationX == 140&&tortoiseLocationY == 280){
			JOptionPane.showMessageDialog(null, "Oh yeah, the treasure is behind that 'Keep Out' sign to the right."
					+ " Sorry about those other two freaks. Don't know what their problem is.");
	
		}
		if((tortoiseLocationX>=560&&tortoiseLocationX<=600)&&(tortoiseLocationY>=200&&tortoiseLocationY<=240)){
		JOptionPane.showMessageDialog(null, "Ya did it. Horray.");
	...
	private void go() {
		...
		Tortoise.penUp();
		JOptionPane.showMessageDialog(null, "Ask the little girl for help with your quest. Press the space bar to ask. P.S, don't touch the fire!");
	}
